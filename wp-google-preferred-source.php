...
class WP_Google_Preferred_Source {
	...
	public function google_news_url_callback() {
		...
		echo '<input type="text" name="' . esc_attr( $this->option_name ) . '[google_news_url]" value="' . $value . '" class="regular-text" placeholder="https://news.google.com/publications/..." />';
	...
	public function auto_append_callback() {
		$options = get_option( $this->option_name );
		$checked = isset( $options['auto_append'] ) && $options['auto_append'] === '1' ? 'checked="checked"' : '';
		echo '<input type="checkbox" name="' . esc_attr( $this->option_name ) . '[auto_append]" value="1" ' . $checked . ' /> Append CTA to the bottom of all posts';
	}
	...
	public function auto_append_to_content( $content ) {
		...
		$options = get_option( $this->option_name );
		if ( isset( $options['auto_append'] ) && $options['auto_append'] === '1' ) {
